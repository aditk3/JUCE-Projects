...
class MidiPlaybackThread : public juce::Thread {
    ...
    */
    ...
    class PlaybackPosition {
        ...
        */
        ...
        PlaybackPosition() : beat(0.0), endbeat(0.0), tick(0.0), index(0), length(0), stop(false) {}
    ...
    */
    class MidiMessageQueue : public juce::OwnedArray<juce::MidiMessage> {
       public:
        ...
        void addMessage(juce::MidiMessage* msg) { addSorted(*this, msg); }
    ...
    */
    MidiPlaybackThread(MidiPlaybackClient* midiClient, int ticksPerBeat, double beatsPerMinute = 60.0,
    ...
    ~MidiPlaybackThread() { messages.clear(); }
    ...
    void setPlaybackPosition(double beat, int index = -1) {
        juce::ScopedLock lock(pblock);
        position.beat = beat;
        if (index > -1) position.index = index;
    }
    ...
    bool isPlaying() { return !isPaused(); }
    ...
    void setPaused(bool pause) {
        ...
        if (paused != pause) {
            ...
            if (!paused)  // wake up thread if we have stopped pausing
                ...
                if (messages.size() > 0) sendAllNotesOff();
            }
        }
    }
...
#pragma GCC diagnostic pop
                ...
                if (isatend) {
...

                    {
                        ...
                        pblock.enter();
                        for (int i = 0; i < localQueue.size(); i++) messages.addMessage(localQueue[i]);
    ...
    void printMidiMessage(juce::MidiMessage& msg) {
        ...
        std::cout << " op=" << juce::String(op) << " ch=" << juce::String(ch) << " d1=" << juce::String(d1)
                  << " d2=" << juce::String(d2) << "\n";
    }
};
