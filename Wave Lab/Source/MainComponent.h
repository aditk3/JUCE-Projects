...
struct MainComponent : public Component,
   ...
   private:
    ...
    StringArray noises{"White", "Brown", "Dust"}, sine{"Sine"}, LF{"LF Impulse", "LF Square", "LF Saw", "LF_Triangle"},
        BL{"BL Impulse", "BL Square", "BL Saw", "BL Triangle"},
        WT{"WT Sine", "WT Impulse", "WT Square", "WT Saw", "WT Triangle"};

    /// A variable holding the currently selected waveform to generate
    /// (see the WaveformId enum).  Its initial value should be Empty.
    WaveformId waveformId{WaveformId::Empty};
    ...
    Slider levelSlider{Slider::SliderStyle::LinearHorizontal, Slider::TextEntryBoxPosition::TextBoxLeft};
    ...
    Slider freqSlider{Slider::SliderStyle::LinearHorizontal, Slider::TextEntryBoxPosition::TextBoxLeft};
