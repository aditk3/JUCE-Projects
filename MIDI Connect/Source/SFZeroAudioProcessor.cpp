...
bool sfzero::SFZeroAudioProcessor::acceptsMidi() const { return true; }

bool sfzero::SFZeroAudioProcessor::producesMidi() const { return false; }
...
void sfzero::SFZeroAudioProcessor::getStateInformation(juce::MemoryBlock &destData) {
    ...
    if (sound) {
        ...
        if (subsound != 0) obj->setProperty("subsound", subsound);
    }
    ...
    if (pathVar.isString()) {
        ...
        if (!sfzFilePath.isEmpty()) {
            ...
            if (sound) {
                ...
                if (subsoundVar.isInt()) sound->useSubsound(int(subsoundVar));
            }
        }
    }
}
